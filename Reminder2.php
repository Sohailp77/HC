            ...
            <div class="panel-body">
                ...
                <table border="0" class="table table-bordered">
                    ...
                    <td align='center' colspan='6'>
                        <input type="radio" name="Task_type" value="T"   onclick="loadPage('Index2.php')">
                        <span id="label_fsearch_case">To Do</span>
                        &nbsp;&nbsp;&nbsp;
                        <input type="radio" name="Task_type" value="R" checked onclick="loadPage('Reminder2.php')">
                ...
                //Daily
                  ...
                  $sql= "SELECT * FROM periphery.task_scheduler WHERE display='Y' AND task_type='R' AND frequency='D' AND  completed !='$fullDate'ORDER BY print_priority DESC";
                ...
                <?php
                    ...
                    echo "<a class='delete-button' href='./delete2.php?id={$row['sr_no']}'>X</a>";
                    echo      "</div>";
                    echo "<a class='btn btn-primary' href='./complete.php?id={$row['sr_no']}'>Completed</a>";
                    echo      "</div>";
                ...
                //weekly
                  ...
                  $sql= "SELECT * FROM periphery.task_scheduler WHERE display='Y' AND task_type='R' AND frequency='W' AND dow='$currentDayOfWeek' AND completed !='$fullDate'ORDER BY print_priority DESC";
                  $sql = $pgconn->prepare($sql); //connecting to database
                  $pgconn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); //pdo of connection
                  $sql->execute(); //to execute the query
                  $rows = $sql->fetchAll();
                ...
                <p></p>
                                    ...
                                    <?php
                                    //Monthly
                                    $sql= "SELECT * FROM periphery.task_scheduler WHERE display='Y' AND task_type='R' AND frequency='M' AND dom='$currentDayOfMonth' AND completed !='$fullDate' ORDER BY print_priority DESC";
                                    $sql = $pgconn->prepare($sql); //connecting to database
                                    $pgconn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); //pdo of connection
                                    $sql->execute(); //to execute the query
                                    $Tasks = $sql->fetchAll();
                                    ...
                                    <?php
                                        ...
                                        echo "<p>{$Task['task_desc']}</p>";
                                        echo "<div class='task-buttons'>";
                                        echo "<a class='delete-button' type=submit href='./delete2.php?id={$Task['sr_no']}'>X</a>";                                       
                                        echo "</div>";
                                        echo "<a class='btn btn-primary' href='./complete.php?id={$Task['sr_no']}'>Completed</a>";
                                        echo "</div>";
                                    ?>
                                    <?php } 
                                    }
                                    ...
                                    ?>                          
                                    <p></p>
                                    <a class="btn btn-primary"  href="Reminder.php">Task Scheduler</a>
